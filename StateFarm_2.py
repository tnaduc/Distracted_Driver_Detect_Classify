...
train_batches = get_batches(path+'train', gen, batch_size=batch_size)
...
model.summary()
batch_size=32
Ex1_history = model.fit(train_data,train_labels, batch_size=batch_size, epochs=5, validation_data =(valid_data,valid_labels))
                
model.fit_generator(batches, len(batches.classes), epochs=2, validation_data=valid_batches, 
                 nb_val_samples=len(valid_batches.classes))				
...
plt.savefig(results_path+'/train_history/Exp2_train_history_1.png', bbox_inches='tight')
plt.show()